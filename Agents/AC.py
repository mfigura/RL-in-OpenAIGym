...
class AC_agent():
    ...
    def _critic_update(self,s,ns,r,n_ep):
        ...
        Arguments: states, new states, rewards, number of training epochs
        '''
        for i in range(n_ep):
            with torch.no_grad():
    ...
    def _actor_update(self,s,ns,r,a):
        '''Actor update'''
        TD_errors = r + self.gamma * self.critic_model(ns) - self.critic_model(s)
        self.actor_model.train()
        a_prob = distributions.Categorical(self.actor_model(s))
    ...
    def update(self,states,new_states,rewards,actions,n_epochs):
        '''Update actor and critic networks'''
        critic_loss = self._critic_update(states,new_states,rewards,n_epochs)
        actor_loss = self._actor_update(states,new_states,rewards,actions)
        return critic_loss, actor_loss

    def get_action(self,state):
        '''Choose an action from the policy at the current state'''
        state = torch.tensor(state,dtype=torch.float32)
        a_prob = distributions.Categorical(self.actor_model(state))
